'use client';

import React, { useState, useEffect } from 'react';
import { Coffee, Trophy, Store, CheckCircle, XCircle, Clock, Flame, Undo2 } from 'lucide-react';

export default function Locals() {
  const [view, setView] = useState('game');
  const [shopCode, setShopCode] = useState('');
  const [wiggle, setWiggle] = useState(false);
  const [timeRemaining, setTimeRemaining] = useState(null);
  const [moveHistory, setMoveHistory] = useState([]);
  const [allCodes, setAllCodes] = useState([]);
  const [shopStats, setShopStats] = useState({ todayCount: 0, totalCount: 0 });
  const [verificationStatus, setVerificationStatus] = useState(null);

  const days = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
  const todayGame = `Sudoku ${days[new Date().getDay()]}`;

  const adjectives = ['Swift', 'Clever', 'Bright', 'Quick', 'Sharp', 'Smart', 'Wise', 'Bold'];
  const nouns = ['Fox', 'Eagle', 'Tiger', 'Wolf', 'Bear', 'Hawk', 'Lion', 'Panda'];
  const colors = ['#d4af37', '#ffd700', '#c9b037', '#b8860b', '#daa520', '#f4c430'];

  const [userId] = useState(() => {
    if (typeof window === 'undefined') return { id: '123', adjective: 'Swift', noun: 'Fox', number: 1234, color: '#d4af37', username: null };
    try {
      const saved = localStorage.getItem('user-id');
      if (saved) return JSON.parse(saved);
      const newUser = {
        id: Math.random().toString(36).substr(2, 9),
        adjective: adjectives[Math.floor(Math.random() * adjectives.length)],
        noun: nouns[Math.floor(Math.random() * nouns.length)],
        number: Math.floor(Math.random() * 9000) + 1000,
        color: colors[Math.floor(Math.random() * colors.length)],
        username: null
      };
      localStorage.setItem('user-id', JSON.stringify(newUser));
      return newUser;
    } catch {
      return { id: '123', adjective: 'Swift', noun: 'Fox', number: 1234, color: '#d4af37', username: null };
    }
  });

  const [userStats, setUserStats] = useState(() => {
    if (typeof window === 'undefined') return { totalCompletions: 0, currentStreak: 0, longestStreak: 0, lastPlayedDate: null, fastestTime: null, completionDates: [], consecutiveRedemptions: 0 };
    try {
      const saved = localStorage.getItem('user-stats');
      return saved ? JSON.parse(saved) : {
        totalCompletions: 0, currentStreak: 0, longestStreak: 0, lastPlayedDate: null,
        fastestTime: null, completionDates: [], consecutiveRedemptions: 0
      };
    } catch { 
      return { totalCompletions: 0, currentStreak: 0, longestStreak: 0, lastPlayedDate: null, fastestTime: null, completionDates: [], consecutiveRedemptions: 0 }; 
    }
  });

  const generatePuzzle = () => {
    const date = new Date().toDateString();
    const seed = date.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
    const base = [[5,3,4,6,7,8,9,1,2],[6,7,2,1,9,5,3,4,8],[1,9,8,3,4,2,5,6,7],[8,5,9,7,6,1,4,2,3],[4,2,6,8,5,3,7,9,1],[7,1,3,9,2,4,8,5,6],[9,6,1,5,3,7,2,8,4],[2,8,7,4,1,9,6,3,5],[3,4,5,2,8,6,1,7,9]];
    const puzzle = base.map(row => [...row]);
    let removed = 0, rng = seed;
    while (removed < 40) {
      rng = (rng * 1103515245 + 12345) & 0x7fffffff;
      const row = rng % 9;
      rng = (rng * 1103515245 + 12345) & 0x7fffffff;
      const col = rng % 9;
      if (puzzle[row][col] !== 0) { puzzle[row][col] = 0; removed++; }
    }
    return { puzzle, solution: base };
  };

  const [gameState, setGameState] = useState(() => {
    if (typeof window === 'undefined') {
      const { puzzle, solution } = generatePuzzle();
      return { 
        board: puzzle.map(row => [...row]), 
        solution, 
        initial: puzzle.map(row => [...row]), 
        selectedCell: null, 
        completed: false, 
        discountCode: null, 
        date: new Date().toDateString(), 
        startTime: Date.now(), 
        completionTime: null,
        discountPercent: null,
        isFree: false
      };
    }
    try {
      const saved = localStorage.getItem('sudoku-state');
      const today = new Date().toDateString();
      if (saved) {
        const parsed = JSON.parse(saved);
        if (parsed.date === today) return parsed;
      }
    } catch {}
    const { puzzle, solution } = generatePuzzle();
    return { 
      board: puzzle.map(row => [...row]), 
      solution, 
      initial: puzzle.map(row => [...row]), 
      selectedCell: null, 
      completed: false, 
      discountCode: null, 
      date: new Date().toDateString(), 
      startTime: Date.now(), 
      completionTime: null,
      discountPercent: null,
      isFree: false
    };
  });

  useEffect(() => { if (typeof window !== 'undefined') { try { localStorage.setItem('sudoku-state', JSON.stringify(gameState)); } catch {} } }, [gameState]);
  useEffect(() => { if (typeof window !== 'undefined') { try { localStorage.setItem('user-stats', JSON.stringify(userStats)); } catch {} } }, [userStats]);

  useEffect(() => {
    const interval = setInterval(() => { setWiggle(true); setTimeout(() => setWiggle(false), 500); }, 60000);
    return () => clearInterval(interval);
  }, []);

  useEffect(() => {
    if (gameState.completed && gameState.completionTime) {
      const updateTimer = () => {
        const now = Date.now();
        const expiresAt = gameState.startTime + gameState.completionTime + (24 * 60 * 60 * 1000);
        const remaining = expiresAt - now;
        if (remaining > 0) {
          const h = Math.floor(remaining / 3600000);
          const m = Math.floor((remaining % 3600000) / 60000);
          const s = Math.floor((remaining % 60000) / 1000);
          setTimeRemaining(`${h}h ${m}m ${s}s`);
        } else {
          setTimeRemaining('EXPIRED');
        }
      };
      updateTimer();
      const timer = setInterval(updateTimer, 1000);
      return () => clearInterval(timer);
    }
  }, [gameState.completed, gameState.completionTime, gameState.startTime]);

  useEffect(() => {
    if (view === 'shop') {
      loadAllCodes();
      loadShopStats();
    }
  }, [view]);

  const loadAllCodes = async () => {
    if (typeof window === 'undefined' || !window.storage) return;
    try {
      const result = await window.storage.list('code:', true);
      if (result && result.keys) {
        const codes = await Promise.all(
          result.keys.map(async (key) => {
            try {
              const data = await window.storage.get(key, true);
              return data ? JSON.parse(data.value) : null;
            } catch {
              return null;
            }
          })
        );
        setAllCodes(codes.filter(c => c !== null).sort((a, b) => b.timestamp - a.timestamp));
      }
    } catch (err) {
      console.log('No codes yet');
      setAllCodes([]);
    }
  };

  const loadShopStats = async () => {
    if (typeof window === 'undefined' || !window.storage) return;
    try {
      const today = new Date().toDateString();
      const todayData = await window.storage.get(`stats:${today}`, true);
      const totalData = await window.storage.get('stats:total', true);
      
      setShopStats({
        todayCount: todayData ? JSON.parse(todayData.value).count : 0,
        totalCount: totalData ? JSON.parse(totalData.value).count : 0
      });
    } catch {
      setShopStats({ todayCount: 0, totalCount: 0 });
    }
  };

  const calculateDiscount = (time) => {
    const min = time / 60000;
    if (min < 3) return 15;
    if (min < 5) return 10;
    return 5;
  };

  const getSeqCode = async () => {
    if (typeof window === 'undefined' || !window.storage) return 1;
    try {
      const today = new Date().toDateString();
      const result = await window.storage.list('code:', true);
      if (result && result.keys) {
        const todayCodes = result.keys.filter(key => {
          const parts = key.split('-');
          return parts.length >= 3;
        });
        return todayCodes.length + 1;
      }
      return 1;
    } catch {
      return 1;
    }
  };

  const selectCell = (row, col) => {
    if (gameState.initial[row][col] !== 0 || gameState.completed) return;
    if (typeof navigator !== 'undefined' && navigator.vibrate) navigator.vibrate(10);
    setGameState(prev => ({ ...prev, selectedCell: prev.selectedCell?.[0] === row && prev.selectedCell?.[1] === col ? null : [row, col] }));
  };

  const handleSubmit = async () => {
    const isComplete = gameState.board.every((row, i) => row.every((cell, j) => cell === gameState.solution[i][j]));
    
    if (isComplete) {
      const compTime = Date.now() - gameState.startTime;
      const date = new Date();
      const seq = await getSeqCode();
      const dateStr = `${(date.getMonth() + 1).toString().padStart(2, '0')}${date.getDate().toString().padStart(2, '0')}`;
      
      const isFree = userStats.consecutiveRedemptions >= 5;
      const disc = isFree ? 100 : calculateDiscount(compTime);
      const code = `LOCAL-${dateStr}-${seq.toString().padStart(3, '0')}`;
      
      const today = date.toDateString();
      let newStreak = 1;
      if (userStats.lastPlayedDate) {
        const last = new Date(userStats.lastPlayedDate);
        const diff = Math.floor((new Date(today) - last) / 86400000);
        if (diff === 1) newStreak = userStats.currentStreak + 1;
        else if (diff > 1) newStreak = 1;
        else newStreak = userStats.currentStreak;
      }
      
      const updated = { 
        totalCompletions: userStats.totalCompletions + 1, 
        currentStreak: newStreak, 
        longestStreak: Math.max(userStats.longestStreak, newStreak), 
        lastPlayedDate: today, 
        fastestTime: userStats.fastestTime ? Math.min(userStats.fastestTime, compTime) : compTime, 
        completionDates: [...userStats.completionDates, today],
        consecutiveRedemptions: userStats.consecutiveRedemptions
      };
      
      setUserStats(updated);
      if (typeof navigator !== 'undefined' && navigator.vibrate) navigator.vibrate([100, 50, 200]);
      
      if (typeof window !== 'undefined' && window.storage) {
        try {
          const codeData = {
            code,
            userId: userId.id,
            userName: getName(),
            userColor: userId.color,
            timestamp: date.getTime(),
            date: today,
            redeemed: false,
            redeemedAt: null,
            discountPercent: disc,
            isFree,
            completionTime: compTime
          };
          
          await window.storage.set(`code:${code}`, JSON.stringify(codeData), true);
        } catch (err) {
          console.error('Failed to save code to shared storage:', err);
        }
      }
      
      setGameState(prev => ({ ...prev, completed: true, discountCode: code, selectedCell: null, completionTime: compTime, discountPercent: disc, isFree }));
    } else {
      if (typeof navigator !== 'undefined' && navigator.vibrate) navigator.vibrate([50, 100, 50]);
      alert('Not quite right! Check your answers and try again.');
    }
  };

  const verifyCode = async () => {
    if (typeof window === 'undefined' || !window.storage) return;
    setVerificationStatus('checking');
    
    try {
      const result = await window.storage.get(`code:${shopCode}`, true);
      
      if (!result) {
        setVerificationStatus({ success: false, message: 'Code not found' });
        return;
      }
      
      const codeData = JSON.parse(result.value);
      
      if (codeData.redeemed) {
        setVerificationStatus({ 
          success: false, 
          message: 'Already redeemed',
          redeemedAt: new Date(codeData.redeemedAt).toLocaleString()
        });
        return;
      }
      
      const expiresAt = codeData.timestamp + (24 * 60 * 60 * 1000);
      if (Date.now() > expiresAt) {
        setVerificationStatus({ success: false, message: 'Code expired' });
        return;
      }
      
      codeData.redeemed = true;
      codeData.redeemedAt = Date.now();
      await window.storage.set(`code:${shopCode}`, JSON.stringify(codeData), true);
      
      const today = new Date().toDateString();
      
      let todayStats = { count: 0 };
      try {
        const todayData = await window.storage.get(`stats:${today}`, true);
        if (todayData) todayStats = JSON.parse(todayData.value);
      } catch {}
      todayStats.count += 1;
      await window.storage.set(`stats:${today}`, JSON.stringify(todayStats), true);
      
      let totalStats = { count: 0 };
      try {
        const totalData = await window.storage.get('stats:total', true);
        if (totalData) totalStats = JSON.parse(totalData.value);
      } catch {}
      totalStats.count += 1;
      await window.storage.set('stats:total', JSON.stringify(totalStats), true);
      
      if (codeData.userId === userId.id) {
        const updatedStats = { ...userStats, consecutiveRedemptions: userStats.consecutiveRedemptions + 1 };
        setUserStats(updatedStats);
      }
      
      setVerificationStatus({ 
        success: true, 
        message: codeData.isFree ? 'FREE COFFEE!' : `${codeData.discountPercent}% OFF`,
        userName: codeData.userName,
        userColor: codeData.userColor,
        isFree: codeData.isFree
      });
      
      if (typeof navigator !== 'undefined' && navigator.vibrate) navigator.vibrate([100, 50, 100, 50, 200]);
      
      setTimeout(() => {
        loadAllCodes();
        loadShopStats();
        setShopCode('');
        setVerificationStatus(null);
      }, 3000);
      
    } catch (err) {
      console.error('Verification error:', err);
      setVerificationStatus({ success: false, message: 'Error verifying code' });
    }
  };

  const enterNumber = (num) => {
    if (!gameState.selectedCell || gameState.completed) return;
    const [row, col] = gameState.selectedCell;
    if (gameState.initial[row][col] !== 0) return;
    
    if (typeof navigator !== 'undefined' && navigator.vibrate) navigator.vibrate(10);
    const oldBoard = gameState.board.map(r => [...r]);
    setMoveHistory(prev => [...prev, { board: oldBoard }]);
    
    const newBoard = gameState.board.map(r => [...r]);
    newBoard[row][col] = num;
    
    setGameState(prev => ({ ...prev, board: newBoard }));
  };

  const undo = () => {
    if (moveHistory.length === 0) return;
    setGameState(prev => ({ ...prev, board: moveHistory[moveHistory.length - 1].board }));
    setMoveHistory(prev => prev.slice(0, -1));
    if (typeof navigator !== 'undefined' && navigator.vibrate) navigator.vibrate(10);
  };

  const reset = () => {
    const { puzzle, solution } = generatePuzzle();
    setGameState({ 
      board: puzzle.map(row => [...row]), solution, initial: puzzle.map(row => [...row]), 
      selectedCell: null, completed: false, discountCode: null, date: new Date().toDateString(), 
      startTime: Date.now(), completionTime: null, discountPercent: null, isFree: false
    });
    setMoveHistory([]);
  };

  const fmt = (ms) => `${Math.floor(ms / 60000)}:${Math.floor((ms % 60000) / 1000).toString().padStart(2, '0')}`;
  const isInit = (r, c) => gameState.initial[r][c] !== 0;
  const isSel = (r, c) => gameState.selectedCell?.[0] === r && gameState.selectedCell?.[1] === c;
  const getName = () => userId.username || `${userId.adjective}${userId.noun}#${userId.number}`;
  const isPuzzleFilled = () => gameState.board.every(row => row.every(cell => cell !== 0));

  if (view === 'streaks') {
    const prog = userStats.consecutiveRedemptions % 6;
    return (
      <div className="min-h-screen bg-gradient-to-br from-stone-50 via-amber-50 to-orange-50 p-4">
        <div className="max-w-md mx-auto">
          <button onClick={() => setView('game')} className="text-stone-600 mb-4">← Back</button>
          <div className="bg-white/80 border-2 border-stone-200 rounded-3xl shadow-2xl p-8">
            <Flame className="w-16 h-16 text-orange-500 mx-auto mb-4" />
            <h2 className="text-3xl font-bold text-stone-900 mb-6 text-center">Streak Rewards</h2>
            
            <div className="bg-orange-50 rounded-2xl p-6 mb-6 border-2 border-orange-200">
              <h3 className="text-xl font-bold mb-2">Daily Reward</h3>
              <p>Complete puzzle → Get <strong>5-15% off</strong> coffee</p>
            </div>

            <div className="bg-green-50 rounded-2xl p-6 mb-6 border-2 border-green-200">
              <h3 className="text-xl font-bold mb-2">🏆 Streak Bonus</h3>
              <p className="mb-2"><strong>Complete & redeem 5 days in a row</strong></p>
              <p className="text-2xl font-black text-green-600">6th coffee FREE! ☕</p>
            </div>

            <div className="mb-4">
              <div className="flex justify-between mb-2">
                <span className="text-sm">Redemption Progress</span>
                <span className="text-sm font-bold text-orange-600">{prog}/5</span>
              </div>
              <div className="flex gap-2">
                {[1,2,3,4,5].map(i => (
                  <div key={i} className={`flex-1 h-3 rounded-full ${i <= prog ? 'bg-orange-500' : 'bg-stone-200'}`}></div>
                ))}
              </div>
              <p className="text-xs text-stone-500 mt-2">Redeem your codes at the counter to progress!</p>
            </div>
          </div>
        </div>
      </div>
    );
  }

  if (view === 'shop') {
    const unredeemed = allCodes.filter(c => !c.redeemed);
    const todayRedeemed = allCodes.filter(c => c.redeemed && c.date === new Date().toDateString());
    
    return (
      <div className="min-h-screen bg-gradient-to-br from-stone-50 via-amber-50 to-orange-50 p-4">
        <div className="max-w-2xl mx-auto">
          <button onClick={() => setView('game')} className="text-stone-600 mb-4">← Back to Game</button>
          
          <div className="bg-white/80 border-2 border-stone-200 rounded-3xl shadow-2xl p-8 mb-6">
            <div className="flex items-center gap-3 mb-6">
              <Store className="w-10 h-10 text-amber-600" />
              <h2 className="text-3xl font-bold">Barista Terminal</h2>
            </div>
            
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-2xl p-4 border-2 border-blue-200">
                <div className="text-sm text-blue-700 mb-1">Today</div>
                <div className="text-3xl font-black text-blue-900">{shopStats.todayCount}</div>
              </div>
              <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-2xl p-4 border-2 border-purple-200">
                <div className="text-sm text-purple-700 mb-1">All Time</div>
                <div className="text-3xl font-black text-purple-900">{shopStats.totalCount}</div>
              </div>
            </div>
            
            <div className="mb-6">
              <input 
                type="text" 
                value={shopCode} 
                onChange={(e) => setShopCode(e.target.value.toUpperCase())} 
                placeholder="LOCAL-0106-001" 
                className="w-full px-6 py-4 bg-stone-50 border-2 rounded-2xl text-center text-2xl font-mono mb-3 focus:border-amber-500 focus:outline-none" 
              />
              <button 
                onClick={verifyCode}
                disabled={!shopCode || verificationStatus === 'checking'}
                className={`w-full py-4 rounded-2xl font-bold text-lg transition-all ${
                  shopCode && verificationStatus !== 'checking'
                    ? 'bg-gradient-to-r from-amber-600 to-orange-600 text-white shadow-lg hover:shadow-xl'
                    : 'bg-stone-200 text-stone-400'
                }`}
              >
                {verificationStatus === 'checking' ? 'Verifying...' : 'Verify Code'}
              </button>
            </div>
            
            {verificationStatus && verificationStatus !== 'checking' && (
              <div className={`rounded-2xl p-6 mb-6 border-2 ${
                verificationStatus.success 
                  ? 'bg-green-50 border-green-300' 
                  : 'bg-red-50 border-red-300'
              }`}>
                <div className="flex items-center gap-3 mb-2">
                  {verificationStatus.success ? (
                    <CheckCircle className="w-8 h-8 text-green-600" />
                  ) : (
                    <XCircle className="w-8 h-8 text-red-600" />
                  )}
                  <div className={`text-2xl font-bold ${
                    verificationStatus.success ? 'text-green-900' : 'text-red-900'
                  }`}>
                    {verificationStatus.message}
                  </div>
                </div>
                {verificationStatus.success && (
                  <div className="flex items-center gap-2 mt-2">
                    <div className="w-3 h-3 rounded-full" style={{ backgroundColor: verificationStatus.userColor }}></div>
                    <span className="text-sm" style={{ color: verificationStatus.userColor }}>
                      {verificationStatus.userName}
                    </span>
                  </div>
                )}
                {!verificationStatus.success && verificationStatus.redeemedAt && (
                  <p className="text-sm text-red-700 mt-2">Redeemed: {verificationStatus.redeemedAt}</p>
                )}
              </div>
            )}
          </div>
          
          <div className="bg-white/80 border-2 border-stone-200 rounded-3xl shadow-2xl p-6 mb-4">
            <h3 className="text-xl font-bold mb-4 flex items-center gap-2">
              <Clock className="w-5 h-5 text-orange-600" />
              Pending Codes ({unredeemed.length})
            </h3>
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {unredeemed.length === 0 ? (
                <p className="text-stone-400 text-center py-8">No pending codes</p>
              ) : (
                unredeemed.map((code) => (
                  <div key={code.code} className="bg-amber-50 rounded-xl p-4 border-2 border-amber-200">
                    <div className="flex justify-between items-start mb-2">
                      <div className="font-mono font-bold text-lg">{code.code}</div>
                      <div className={`px-3 py-1 rounded-full text-sm font-bold ${
                        code.isFree 
                          ? 'bg-green-500 text-white' 
                          : 'bg-orange-500 text-white'
                      }`}>
                        {code.isFree ? 'FREE' : `${code.discountPercent}%`}
                      </div>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <div className="w-2 h-2 rounded-full" style={{ backgroundColor: code.userColor }}></div>
                      <span style={{ color: code.userColor }}>{code.userName}</span>
                      <span className="text-stone-400">•</span>
                      <span className="text-stone-600">{new Date(code.timestamp).toLocaleTimeString()}</span>
                    </div>
                  </div>
                ))
              )}
            </div>
          </div>
          
          <div className="bg-white/80 border-2 border-stone-200 rounded-3xl shadow-2xl p-6">
            <h3 className="text-xl font-bold mb-4 flex items-center gap-2">
              <CheckCircle className="w-5 h-5 text-green-600" />
              Redeemed Today ({todayRedeemed.length})
            </h3>
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {todayRedeemed.length === 0 ? (
                <p className="text-stone-400 text-center py-8">No redemptions yet today</p>
              ) : (
                todayRedeemed.map((code) => (
                  <div key={code.code} className="bg-green-50 rounded-xl p-3 border border-green-200">
                    <div className="flex justify-between items-center">
                      <div className="font-mono text-sm">{code.code}</div>
                      <div className="text-xs text-green-700">
                        {new Date(code.redeemedAt).toLocaleTimeString()}
                      </div>
                    </div>
                  </div>
                ))
              )}
            </div>
          </div>
        </div>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-gradient-to-br from-stone-50 via-amber-50 to-orange-50 p-4 flex flex-col items-center justify-center">
      <div className="max-w-md w-full">
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-4 mb-3">
            <Coffee className="w-12 h-12 text-amber-600" />
            <h1 className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-700 to-orange-600">Locals</h1>
          </div>
          
          <p className="text-sm font-medium text-stone-600 mb-3">🔢 {todayGame}</p>
          <div className="flex items-center justify-center gap-2 mb-3">
            <div className="w-2 h-2 rounded-full" style={{ backgroundColor: userId.color }}></div>
            <span className="text-sm font-bold" style={{ color: userId.color }}>{getName()}</span>
          </div>
          
          <p className={`text-amber-700 font-bold text-lg ${wiggle ? 'wiggle' : ''}`}>Complete for exclusive discount</p>
          <style>{`@keyframes wiggle{0%,100%{transform:rotate(0)}25%{transform:rotate(-3deg)}75%{transform:rotate(3deg)}}.wiggle{display:inline-block;animation:wiggle .5s}`}</style>
          
          <div className="flex justify-center gap-6 mt-4">
            <div className="flex items-center gap-2"><Flame className="w-5 h-5 text-orange-500" /><span className="font-bold text-lg">{userStats.currentStreak}</span></div>
            <div className="flex items-center gap-2"><Trophy className="w-5 h-5 text-amber-600" /><span className="font-bold text-lg">{userStats.totalCompletions}</span></div>
          </div>
          
          <div className="flex justify-center gap-2 mt-4">
            <button onClick={() => setView('streaks')} className="px-4 py-2 rounded-xl bg-orange-500 text-white font-medium">Streaks</button>
            <button onClick={() => setView('shop')} className="px-4 py-2 rounded-xl bg-stone-700 text-white font-medium">☕ Barista</button>
          </div>
        </div>

        {gameState.completed && (
          <div className="bg-white/80 border-2 border-stone-200 rounded-3xl shadow-2xl p-8 mb-6 text-center">
            <div className="w-20 h-20 mx-auto mb-4 rounded-full bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center">
              <Trophy className="w-12 h-12 text-white" />
            </div>
            <h2 className="text-3xl font-bold mb-2">Victory!</h2>
            <p className="text-stone-600 mb-4">Completed in {fmt(gameState.completionTime)}</p>
            <div className="bg-gradient-to-r from-amber-600 to-orange-600 rounded-2xl p-6 mb-4 shadow-xl">
              <div className="text-3xl font-mono font-bold text-white mb-2">{gameState.discountCode}</div>
              {gameState.isFree ? (
                <div className="text-3xl font-black text-white">FREE COFFEE ☕</div>
              ) : (
                <div className="text-2xl font-black text-white">{gameState.discountPercent}% OFF</div>
              )}
            </div>
            <p className="text-xs text-stone-500 mb-1">Show to barista when ordering</p>
            <p className="text-xs text-stone-500 mb-4">Code saved automatically</p>
            <div className="flex items-center justify-center gap-2 text-sm mb-6">
              <Clock className="w-4 h-4" />
              <span className={timeRemaining === 'EXPIRED' ? 'text-red-600' : 'text-amber-700'}>Valid: {timeRemaining || '...'}</span>
            </div>
            <button onClick={reset} className="bg-stone-800 text-white px-8 py-3 rounded-xl font-bold">New Puzzle</button>
          </div>
        )}

        <div className="bg-white/80 border-2 border-stone-200 rounded-3xl shadow-2xl p-6 mb-4">
          <div className="grid grid-cols-9 gap-0 border-4 border-stone-300 rounded-2xl overflow-hidden">
            {gameState.board.map((row, i) => row.map((cell, j) => (
              <div 
                key={`${i}-${j}`} 
                onClick={() => selectCell(i, j)} 
                className={`aspect-square flex items-center justify-center text-lg font-bold
                  ${(j + 1) % 3 === 0 && j !== 8 ? 'border-r-4 border-stone-400' : 'border-r border-stone-300'}
                  ${(i + 1) % 3 === 0 && i !== 8 ? 'border-b-4 border-stone-400' : 'border-b border-stone-300'}
                  ${isInit(i, j) ? 'bg-stone-100 text-stone-800' : 'bg-white text-amber-700'}
                  ${isSel(i, j) ? 'bg-amber-100 ring-2 ring-amber-500' : ''}
                  ${!isInit(i, j) && !gameState.completed ? 'cursor-pointer hover:bg-amber-50' : ''}`}
              >
                {cell !== 0 ? cell : ''}
              </div>
            )))}
          </div>
        </div>

        {!gameState.completed && (
          <div className="bg-white/80 border-2 border-stone-200 rounded-3xl shadow-2xl p-6">
            <div className="flex justify-between items-center mb-4">
              <button onClick={undo} disabled={moveHistory.length === 0} className={`flex items-center gap-2 px-4 py-2 rounded-xl font-bold ${moveHistory.length > 0 ? 'bg-blue-100 text-blue-700 border-2 border-blue-300' : 'bg-stone-100 text-stone-400 border-2 border-stone-200'}`}>
                <Undo2 className="w-4 h-4" /> Undo
              </button>
              <span className="text-sm text-stone-600">Moves: {moveHistory.length}</span>
            </div>
            <div className="grid grid-cols-5 gap-2 mb-4">
              {[1,2,3,4,5,6,7,8,9].map(n => (
                <button key={n} onClick={() => enterNumber(n)} disabled={!gameState.selectedCell} className={`aspect-square rounded-xl text-xl font-bold ${gameState.selectedCell ? 'bg-gradient-to-br from-amber-600 to-orange-600 text-white shadow-lg' : 'bg-stone-100 text-stone-400 border-2 border-stone-200'}`}>{n}</button>
              ))}
              <button onClick={() => enterNumber(0)} disabled={!gameState.selectedCell} className={`aspect-square rounded-xl text-sm font-bold ${gameState.selectedCell ? 'bg-red-100 text-red-700 border-2 border-red-300' : 'bg-stone-100 text-stone-400 border-2 border-stone-200'}`}>Clear</button>
            </div>
            
            {isPuzzleFilled() && (
              <button 
                onClick={handleSubmit}
                className="w-full bg-gradient-to-r from-green-600 to-emerald-600 text-white py-4 rounded-2xl font-bold text-lg shadow-lg hover:shadow-xl transition-all hover:scale-[1.02] active:scale-[0.98]"
              >
                Submit Answer ✓
              </button>
            )}
          </div>
        )}
      </div>
    </div>
  );
}
